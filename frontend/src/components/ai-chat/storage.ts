...
export function readConversations(): Conversation[] {
  ...
  const raw = readStorageValue(CHAT_CONVERSATIONS_STORAGE_KEY);
  if (!raw) {
    return fallback;
  }
...
export function writeConversations(conversations: Conversation[]) {
  writeStorageValue(CHAT_CONVERSATIONS_STORAGE_KEY, JSON.stringify(conversations));
}

export function readActiveConversationId() {
  return readStorageValue(CHAT_ACTIVE_CONVERSATION_STORAGE_KEY) || "";
}

export function writeActiveConversationId(conversationId: string) {
  writeStorageValue(CHAT_ACTIVE_CONVERSATION_STORAGE_KEY, conversationId);
}

export function readSelectedModelId() {
  return readStorageValue(CHAT_SELECTED_MODEL_STORAGE_KEY) || "";
}

export function writeSelectedModelId(modelId: string) {
  writeStorageValue(CHAT_SELECTED_MODEL_STORAGE_KEY, modelId);
}
