        ...
        <table className="task-table">
          ...
          <tbody>
            {tasks.map((task, index) => (
              <tr className={`task-table-row ${task.status === "已完成" ? "is-complete" : ""} ${task.priority === "高" ? "is-high-priority" : ""} ${openMenuTaskId === task.id ? "is-menu-open row-has-open-menu" : ""}`} key={task.id} style={{ "--stagger-index": index } as CSSProperties} onClick={() => { setOpenMenuTaskId(null); onOpenTask(task); }}>
